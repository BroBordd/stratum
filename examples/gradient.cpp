...
void main() {
    float d = vUV.x * 0.5 + vUV.y * 0.5;
    float h1 = mod(time * 0.1 + d * 0.4, 1.0);
    float h2 = mod(h1 + 0.5, 1.0);
    vec3 col1 = hsv2rgb(vec3(h1, 0.8, 1.0));
    vec3 col2 = hsv2rgb(vec3(h2, 0.8, 1.0));
    gl_FragColor = vec4(mix(col1, col2, d), 1.0);
}
)";
