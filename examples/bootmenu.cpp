...
enum class Screen { MENU, CONFIRM };
...
int main(int argc, char** argv) {
    ...
    s.onKey([&](const KeyEvent& e) {
        ...
        if (screen == Screen::MENU) {
            ...
            if (e.code == KEY_POWER) {
                pendingIdx = menuSel; dlgSel = 0; screen = Screen::CONFIRM;
            }
        } else {
    ...
    s.onFrame([&](float t) {
        ...
        Screen curScreen; int curMenuSel, curDlgSel, curPending, curFlashIdx;
        float  curFlashT;
        {
            std::lock_guard<std::mutex> lk(mtx);
            curScreen   = screen;
        ...
        // progress bar — ease-in-out sine, fast body slow approach, period ~1.6s
        {
            ...
            // t in [0,1] over period, eased with smoothstep-like sine:
            // pos = 0.5 - 0.5*cos(pi*t) gives slow-fast-slow (ease in-out)
            // we want fast body, slow only at each end → use a sharper curve:
            // pos = (sin(phase - pi/2) + 1) / 2  with a compressed linear mid
            // achieved cleanly with: pos = smoothstep variant on a 1.6s period
            float period  = 1.6f;
            float elapsed = now - startTime;
            // ping-pong: fold elapsed into [0, period]
            float cycle   = fmodf(elapsed, period * 2.0f);
            float u       = (cycle < period) ? cycle / period : 1.0f - (cycle - period) / period;
            // apply ease: slow at ends, faster through middle
            // use smootherstep: 6t^5 - 15t^4 + 10t^3
...

        {
            ...
            // push hint up so copyright fits below without overlap
            float hintY = 0.945f - fs2;
            float copyY = hintY + fs2 + 0.004f;
            Text::draw(hint, 0.05f, hintY, fs2, 0.18f*dimA, 0.20f*dimA, 0.26f*dimA);
            float cpW = strlen(copy) * vs2 / asp;
            float cpX = 0.95f - cpW;
            Text::draw(copy, cpX, 0.975f, vs2, 0.16f*dimA, 0.22f*dimA, 0.32f*dimA);
        }
